...
use vicis_core::ir::{
    function::{
        ...
        data::Data as IrData,
        instruction::{
            Alloca, Br, Call, CondBr, ICmp, ICmpCond,
...
fn lower_bin(
    ...
    let insert_not = |ctx: &mut LoweringContext<Ola>| {
        ctx.inst_seq.push(MachInstruction::new(
            InstructionData {
                opcode: Opcode::NOT,
                operands: vec![MO::output(OperandData::Reg(GR::R7.into())), MO::input(rhs.clone().into())],
            },
            ctx.block_map[&ctx.cur_block],
        ))
    };
...
fn lower_condbr(
    ...
    if let Some((icmp, ty, args, cond)) = is_icmp(ctx.ir_data, arg) {
                ...
                match cond {
                    ...
                    ICmpCond::Ult  => {
                        ...
                        ctx.inst_seq.push(MachInstruction::new(
                            InstructionData {
                                opcode: Opcode::MOVri,
                                operands: vec![MO::output(OperandData::VReg(output)), MO::new(rhs.into())],
                            },
                            ctx.block_map[&ctx.cur_block],
                        ));
                        ...
                        ctx.inst_seq.push(MachInstruction::new(
                            InstructionData {
                                opcode: Opcode::GTE,
                                operands: vec![MO::input(OperandData::VReg(output)), MO::input(lhs.into())],
                            },
                            ctx.block_map[&ctx.cur_block],
                        ));
        ...
        ctx.inst_seq.push(MachInstruction::new(
            InstructionData {
                opcode: Opcode::CJMPr/*match cond {
